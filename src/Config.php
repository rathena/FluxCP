...
/**
 * Config acts as a convenient way to access arrays which are used as a
...
class Config
{
	 ...
	 * class to do so. This will cause Config to raise the exception class
	 * of your choice.
	 *
	 ...
	 * @var string
	 */
	private $exceptionClass = 'rAthena\FluxCp\Error';

	/**
	 * Construct a Config instance which acts as a more convenient
	 ...
	 * them collectively as an array of Config instances.
	 *
	 * @return array Array of Config instances of all children arrays.
	 * @access public
	 */
	 ...
	 * will be returned as an instance of Config by default, unless
	 ...
	 * @param bool $configObjectIfArray True/false whether or not to return Config instances for values that are an array.
	 * @access public
	 */
	public function get($key, $configObjectIfArray = true)
	{
		$keys = explode('.', $key);
		$base = &$this->configArray;
		$size = count($keys) - 1;
	 ...
	 * is 100% identical to Config::get().
	 *
	 * Options outline:
	...
	public function merge(Config $config, $recursive = true)
	{
		...
		$this->configArray = $mergeMethod($this->configArray, $config->toArray());
	}
}
