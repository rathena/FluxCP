...
class Dispatcher
{
	...
	public function dispatch($options = array())
	{
		...
		if (!$defaultModule) {
			throw new Error('Please set the default module with $dispatcher->setDefaultModule()');
		} elseif (!$defaultAction) {
			throw new Error('Please set the default action with $dispatcher->setDefaultAction()');
		}
