...
/**
 * The rAthena\FluxCp\Flux class contains methods related to the application on the larger
...
class Flux
{
	...
	/**
	 * Registry where LoginAthenaGroup instances are kept for easy
	 * searching.
	 *
	 * @access public
	 * @var array
	...
	/**
	 * Registry where Athena instances are kept for easy searching.
	 *
	 * @access public
	 * @var array
	 */
	 ...
	 * @throws Error Raised when missing required options.
	 * @access public
	 */
	...
	public static function initializeServerObjects()
	{
		foreach (self::$serversConfig->getChildrenConfigs() as $key => $config) {
			$connection = new Connection($config->getDbConfig(), $config->getLogsDbConfig());
			$loginServer = new LoginServer($config->getLoginServer());

			// LoginAthenaGroup maintains the grouping of a central login
			// server and its underlying Athena objects.
			self::$servers[$key] = new LoginAthenaGroup($config->getServerName(), $connection, $loginServer);
			...
			foreach ($config->getCharMapServers()->getChildrenConfigs() as $charMapServer) {
				$charServer = new CharServer($charMapServer->getCharServer());
				$mapServer = new MapServer($charMapServer->getMapServer());

				// Create the collective server object, Athena.
				$athena = new Athena($charMapServer, $loginServer, $charServer, $mapServer);
	...
	public static function initializeAddons()
	{
		...
		foreach (glob(FLUX_ADDON_DIR . '/*') as $addonDir) {
			if (is_dir($addonDir)) {
				...
				$addonObject = new Addon($addonName, $addonDir);
	...
	/**
	 * Convenience method for raising Error exceptions.
	 *
	...
	/**
	 * Parse PHP array into Config instance.
	 *
	...
	public static function parseAppConfigFile($filename)
	{
		...
		if (!($config->getPayPalReceiverEmails() instanceOf Config)) {
	 ...
	 * @param LoginAthenaGroup Server group object.
	 * @return LoginAthenaGroup
	 * @access private
	 */
	private static function registerServerGroup($serverName, LoginAthenaGroup $serverGroup)
	{
	 ...
	 * @param Athena $athenaServer Athena server object.
	 * @return Athena
	 * @access private
	 */
	private static function registerAthenaServer($serverName, $athenaServerName, Athena $athenaServer)
	{
	...
	/**
	 * Get LoginAthenaGroup server object by its ServerName.
	 *
	 * @param string $serverName Server group name.
	 * @return mixed Returns LoginAthenaGroup instance or false on failure.
	 * @access public
	 */
	public static function getServerGroupByName($serverName)
	{
		...
		if (array_key_exists($serverName, $registry) && $registry[$serverName] instanceOf LoginAthenaGroup) {
	...
	/**
	 * Get Athena instance by its group/server names.
	 *
	 * @param string $serverName Server group name.
	 * @param string $athenaServerName Athena server name.
	 * @return mixed Returns Athena instance or false on failure.
	 * @access public
	 */
	public static function getAthenaServerByName($serverName, $athenaServerName)
	{
		...
		if (array_key_exists($serverName, $registry) && array_key_exists($athenaServerName, $registry[$serverName]) &&
			$registry[$serverName][$athenaServerName] instanceOf Athena) {
