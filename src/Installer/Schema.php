...
class Schema
{
		...
		if (!array_key_exists($version, $this->schemaInfo['versions']) || !$this->schemaInfo['versions'][$version]) {
			...
			if ($sth->errorCode()) {
				...
				if (!empty($errnum) && $errnum == 1045) {
					throw new Error("Critical MySQL error in Installer/Updater: $errnum: $errmsg");
				} elseif (!empty($errnum) && $errnum == 1142) {
					// Bail-out.
					$message = "MySQL error: $errmsg\n\n";
					throw new SchemaPermissionError(
						$message,
					...
					throw new Error("Critical MySQL error in Installer/Updater: $errmsg");
				}
			}
	 ...
	 *
	 */
	public static function getSchemas(MainServer $mainServer, CharMapServer $charMapServer = null)
	{
