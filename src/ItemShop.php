...
class ItemShop
{
	...
	public function __construct(Athena $server)
	{
		$this->server = $server;
	}
	 ...
	 *
	 */
	public function buy(DataObject $account, $shopItemID)
	{
	...
	public function getItem($shopItemID)
	{
		...
		$temp = new TemporaryTable($this->server->connection, "$db.items", $fromTables);
	 ...
	 *
	 */
	public function uploadShopItemImage($shopItemID, Config $file)
	{
		if ($file->get('rAthena\FluxCp\Error')) {
			return false;
		}
