...
use AccountLevel;
use rAthena\FluxCp\ItemShop\Cart;
...
class SessionData
{
	...
	private function initialize($force = false)
	{
		...
		if ($this->loginAthenaGroup && $this->username && ($account = $this->getAccount($this->loginAthenaGroup, $this->username))) {
			...
			$this->account = new DataObject(null, array('group_level' => AccountLevel::UNAUTH));
		}
		...
		if ($this->account->account_id && $this->loginAthenaGroup) {
			...
			foreach ($this->getAthenaServerNames() as $athenaServerName) {
				...
				if (!array_key_exists($athenaServerName, $cartArray[$accountID])) {
					$cartArray[$accountID][$athenaServerName] = new Cart();
				}
	...
	public function login($server, $username, $password, $securityCode = null)
	{
		...
		if (!$loginAthenaGroup) {
			throw new LoginError('Invalid server.', LoginError::INVALID_SERVER);
		}

		if ($loginAthenaGroup->loginServer->isIpBanned() && !Flux::config('AllowIpBanLogin')) {
			throw new LoginError('IP address is banned', LoginError::IPBANNED);
		}

		if ($securityCode !== false && Flux::config('UseLoginCaptcha')) {
			if (strtolower($securityCode) != strtolower($this->securityCode)) {
				throw new LoginError('Invalid security code', LoginError::INVALID_SECURITY_CODE);
		...
		if (!$loginAthenaGroup->isAuth($username, $password)) {
			throw new LoginError('Invalid login', LoginError::INVALID_LOGIN);
		}
		...
		if ($res && ($row = $smt->fetch())) {
			if ($row->unban_time > 0) {
				if (time() >= $row->unban_time) {
					...
					throw new LoginError('Temporarily banned', LoginError::BANNED);
				}
			}
			if ($row->state == 5) {
				...
				if ($row2 && $row2->id) {
					throw new LoginError('Pending confirmation', LoginError::PENDING_CONFIRMATION);
				}
			}
			if (!Flux::config('AllowPermBanLogin') && $row->state == 5) {
				throw new LoginError('Permanently banned', LoginError::PERMABANNED);
			}
			...
			throw new LoginError($message, LoginError::UNEXPECTED);
		}
	 ...
	 *
	 * @param LoginAthenaGroup $loginAthenaGroup
	 * @param string $username
	 * @return mixed
	 * @access private
	 */
	private function getAccount(LoginAthenaGroup $loginAthenaGroup, $username)
	{
		$creditsTable = Flux::config('FluxTables.CreditsTable');
		$creditColumns = 'credits.balance, credits.last_donation_date, credits.last_donation_amount';
