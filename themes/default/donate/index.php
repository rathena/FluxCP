...
$bothPayments     = (in_array('stripe', Flux::config('PaymentGateway')->toArray()) && in_array('paypal', Flux::config('PaymentGateway')->toArray()) ? true : false);
$paymentGatewayTxt = ($bothPayments ? 'PayPal or Stripe' : (in_array('stripe', Flux::config('PaymentGateway')->toArray()) ? 'Stripe' : 'PayPal'));
?>
...
<?php if (Flux::config('AcceptDonations')): ?>
	...
	<p>All donations towards us are received by <?php echo $paymentGatewayTxt ?>, but don't worry!  Even if you don't have an account with <?php echo $paymentGatewayTxt ?>, you can still use your credit card to donate!</p>
	...
	<?php else: ?>
        <?php if (in_array('paypal', Flux::config('PaymentGateway')->toArray())): ?>
        ...
        <?php endif ?>
        <?php if (in_array('stripe', Flux::config('PaymentGateway')->toArray())): ?>
            <p>Or you can donate with Stripe and try others methods</p>
        <?php endif ?>
